...
function key_set(list, key3, value4) {
  if (list.hasLength(0)) {
    return toList([[key3, value4]]);
  } else if (list.atLeastLength(1) && isEqual(list.head[0], key3)) {
    let k = list.head[0];
    let rest$1 = list.tail;
...
function assocCollision(root2, shift, hash, key3, val, addedLeaf) {
  if (hash === root2.hash) {
    ...
    return {
      type: COLLISION_NODE,
      hash,
      array: cloneAndSet(root2.array, size, { type: ENTRY, k: key3, v: val })
    };
  }
  return assoc(
    {
...
function map_get(map6, key3) {
  const value4 = map6.get(key3, NOT_FOUND);
  if (value4 === NOT_FOUND) {
    return new Error2(Nil2);
  }
  return new Ok2(value4);
}
function map_insert(key3, value4, map6) {
  return map6.set(key3, value4);
}
...
function insert(dict, key3, value4) {
  return map_insert(key3, value4, dict);
}
...
function append4(first4, second2) {
  let _pipe = first4;
...
var LustreClientApplication2 = class _LustreClientApplication {
  ...
  #update = null;
  #view = null;
  static start(flags, selector, init6, update4, view3) {
    if (!is_browser())
      return new Error2(new NotABrowser());
    const root2 = selector instanceof HTMLElement ? selector : document.querySelector(selector);
    if (!root2)
      return new Error2(new ElementNotFound(selector));
    const app = new _LustreClientApplication(init6(flags), update4, view3, root2);
...
var App = class extends CustomType {
  constructor(init6, update4, view3, on_attribute_change) {
    super();
    this.init = init6;
    this.update = update4;
    this.view = view3;
...
function application(init6, update4, view3) {
  return new App(init6, update4, view3, new None());
}
...
function deepEqual(a, b) {
  ...
  if (consts.includes(typeof a) || consts.includes(typeof b))
    return a === b;
  for (const value4 in a) {
    if (!(value4 in b))
      return false;
    const isSame = deepEqual(a[value4], b[value4]);
    if (!isSame)
      return false;
  }
  return true;
...
function shadow(root2) {
  return new Options(new Shadow(root2));
}
...
function top(size) {
  return property2("top", to_string6(size));
}
function bottom(size) {
  return property2("bottom", to_string6(size));
}
function right(size) {
  return property2("right", to_string6(size));
}
function left(size) {
  return property2("left", to_string6(size));
}
...
function margin(margin2) {
  return property2("margin", to_string6(margin2));
}
...
function dynamic(id, styles) {
  let _pipe = styles;
  let _pipe$1 = convert_styles(_pipe);
  return compileClass(_pipe$1, id);
}
...
function replace3(debuggers, debugger_, mapper) {
  ...
  let _pipe$2 = unwrap2(_pipe$1, [debugger_, init3()]);
...
function inspectDict2(map6) {
  const data = [];
  map6.forEach((value4, key3) => data.push([inspect3(key3), inspect3(value4)]));
...
function setup2() {
  ...
  let _pipe$2 = map3(
    _pipe$1,
    (_capture) => {
      return application(init4, update2, _capture);
    }
  );
  let _pipe$3 = try$(
    _pipe$2,
...
// build/dev/javascript/inputs_focus_repro/inputs_focus_repro.mjs
var AddedInput = class extends CustomType {
};
var UpdatedCatAtIndex = class extends CustomType {
  constructor(x0, x1) {
    super();
    this[0] = x0;
    this[1] = x1;
...
function update3(model, msg) {
  if (msg instanceof AddedInput) {
    return [
      new Model2(
        insert(model.cats, map_size(model.cats), new None())
      ),
      none()
    ];
  } else if (msg instanceof UpdatedCatAtIndex) {
    let idx = msg[0];
    let newcat = msg[1];
    return [
      new Model2(
        update(model.cats, idx, (_) => {
          return new Some(newcat);
        })
      ),
      none()
    ];
    ...
    let newdict = msg[0];
    return [new Model2(newdict), none()];
  }
}
function view_cat_input(index3, cat) {
  let update_cat_with_index = curry2(
    (var0, var1) => {
      return new UpdatedCatAtIndex(var0, var1);
    }
  );
  return input(
    toList([
      name("input-" + to_string3(index3)),
      on_input(update_cat_with_index(index3)),
      value(
        (() => {
          if (cat instanceof Some) {
            let cat$1 = cat[0];
            return cat$1;
          } else {
            return "";
...
function view2(model) {
  return div(
    ...
    toList([
      ...
      button(
        toList([on_click(new AddedInput())]),
        toList([text2("Add another cat")])
      )
    ])
  );
}
function init5(_) {
  return [
    new Model2(from_list(toList([[0, new Some("Puss in boots")]]))),
    none()
  ];
}
function main() {
  ...
  let main$1 = $[0];
  let _pipe = application(init5, update3, view2);
